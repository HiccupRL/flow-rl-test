...
LOG_TAG="${LOG_TAG:-mujoco-onpolicy-200m-4seed}"
LOG_GROUP="${LOG_GROUP:-mujoco-onpolicy-baseline-200m-4seed}"
RUN_LOG_ROOT="${RUN_LOG_ROOT:-log/parallel/${LOG_GROUP}}"
...
stop_all() {
    ...
    STOPPING=1
    trap - INT TERM
    echo "[$(date '+%F %T')] stopping MuJoCo on-policy workers ..."
    for pid in "${PIDS[@]:-}"; do
        kill_tree "${pid}" TERM
    done
    sleep 3
        ...
        for task in "${TASKS[@]}"; do
            for seed in "${SEEDS[@]}"; do
                if (( job_idx % num_workers == worker_idx )); then
                    run_one "${algo}" "${task}" "${seed}" "${gpu}" "$((job_idx + 1))" || status=1
                fi
                ((job_idx += 1))
            done
        done
...
main() {
    ...
    if (( status == 0 )); then
        echo "[$(date '+%F %T')] all MuJoCo on-policy runs completed"
    else
        echo "[$(date '+%F %T')] some MuJoCo on-policy runs failed" >&2
    fi
    exit "${status}"
}
